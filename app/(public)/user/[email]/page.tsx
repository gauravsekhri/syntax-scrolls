...
const SettingsPage = async ({ params }: { params: { email: string } }) => {
  ...
  const [userDetails, userPosts]: any = await Promise.all([
    getUserDetails(currEmail),
    getUserPosts(currEmail),
  ]);
    ...
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 px-4 sm:px-24 mb-20">
        {userPosts?.map((postElem: any) => (
          <div className="relative p-2 pt-10 sm:py-6 sm:pl-6 sm:pr-7 bg-card rounded-lg border border-slate-300 dark:border-none flex flex-col min-h-44 sm:min-h-56">
